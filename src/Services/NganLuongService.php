...
class NganLuongService
{
    ...
    public $nganluong_url = '';

    // Mã website của bạn đăng ký trong chức năng tích hợp thanh toán của NgânLượng.vn.
    public $merchant_site_code = 51197; //100001 chỉ là ví dụ, bạn hãy thay bằng mã của bạn

    // Mật khẩu giao tiếp giữa website của bạn và NgânLượng.vn.
    public $secure_pass = '98ad27e6b405b8e30e498d7d1a9cb4a4'; //d685739bf1 chỉ là ví dụ, bạn hãy thay bằng mật khẩu của bạn
    ...
    private $merchant_email = 'nguyentrongpho@gmail.com';

    private $debug = true;

    public function __construct()
    {
        $this->nganluong_url = 'https://sandbox.nganluong.vn:8088/nl35/checkout.php';
        $this->merchant_site_code = '51197';
        $this->secure_pass = '98ad27e6b405b8e30e498d7d1a9cb4a4';
        $this->merchant_email = 'nguyentrongpho@gmail.com';
    }
     ...
     * @return string
     */
    public function buildCheckoutUrlExpand($return_url, $transaction_info, $order_code, $price, $currency = 'vnd', $quantity = 1, $tax = 0, $discount = 0, $fee_cal = 0, $fee_shipping = 0, $order_description = '', $buyer_info = '', $affiliate_code = '')
    {
        if ($affiliate_code == '') {
        ...
        $arr_param = [
            ...
            'receiver' => strval($this->merchant_email),
    ...
    // public function buildInchiCheckoutUrl($orderMetaData, $cartData)
    // {
    //     $params = [
    //         //Mã đơn hàng
    //         'order_code' => 'WEB_' . $orderMetaData['order_id'],
    //         //Khai báo url trả về
    //         'return_url' => base_url('/dat-hang/thanh-cong?id=' . $orderMetaData['order_id']),
    //         // Link nut hủy đơn hàng
    //         'cancel_url' => base_url('/dat-hang/huy-don?id=' . $orderMetaData['order_id']),
    //         'notify_url' => base_url('/checkout/ipn'),
    //         //Giá của cả giỏ hàng
    //         'price' => (int) $cartData['total'],
    //         'currency' => 'vnd',
    //         //Thông tin giao dịch
    //         'transaction_info' => 'Giao dich Ngan Luong',
    //         'quantity' => count($cartData['items']),
    //         'tax' => 0,
    //         'discount' => 0,
    //         'fee_cal' => 0,
    //         'fee_shipping' => 0,
    //         'order_description' => '',
    //         'buyer_info' => $orderMetaData['billing_name'] . "*|*" . @$orderMetaData['billing_email'] . "*|*" . $orderMetaData['billing_phone'],
    //     ];

    //     foreach ($cartData['items'] as $key => $item) {
    //         $params['order_description'] .= $item['qty'] . ' x ' . $item['product_name'] . ' + ';
    //     }
    ...
    //     return $url;
    // }
    public function buildBrvCheckoutUrl($data)
    {
        $params = [
            //Mã đơn hàng
            ...
            // Link nut hủy đơn hàng
            'cancel_url' => route('cancel', ['id' => $data['order']->id]),
            'notify_url' => route('home'),
            //Giá của cả giỏ hàng
            'price' => (int) $data['meta']['price'],
            'currency' => 'vnd',
            //Thông tin giao dịch
            'transaction_info' => 'Giao dich Ngan Luong',
            'quantity' => $data['meta']['qty'],
            'tax' => 0,
            'discount' => 0,
            'fee_cal' => 0,
            'fee_shipping' => 0,
